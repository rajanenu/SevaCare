...
class _QrAppointmentFormScreenState
      ...
      if (mounted) {
        setState(() {
          _loadError = extractErrorMessage(
            e,
            fallback:
                'Failed to load form data. Please scan the QR code again.',
          );
          _loading = false;
        });
      }
  ...
  Future<void> _submit() async {
    ...
    if (mobileDigits.length != 10) {
      setState(
        () => _submitError = 'Please enter a valid 10-digit mobile number.',
      );
      return;
    }
    ...
    try {
      ...
      final specialty =
          doctorInfo['specialty'] as String? ?? _selectedSpecialty ?? '';
      final result = await repo.submitQrAppointmentRequest(widget.qrcodeUuid, {
      ...
      if (mounted) {
        setState(() {
          ...
          _submitError = extractErrorMessage(
            e,
            fallback: 'Failed to submit request. Please try again.',
          );
        });
      }
    }
  ...
  Widget build(BuildContext context) {
          ...
          child: Center(
            child: ConstrainedBox(
              constraints: BoxConstraints(
                maxWidth: contentMaxWidthFor(MediaQuery.sizeOf(context).width),
              ),
              child: Column(
    ...
    final specialties =
        _doctors.map((d) => d['specialty'] as String? ?? '').toSet().toList()
          ..sort();
                        ...
                        if (filteredDoctors.isEmpty)
                          ...
                          Column(
                            children: filteredDoctors.map((doc) {
                              ...
                              final name =
                                  doc['doctorName'] as String? ?? 'Doctor';
