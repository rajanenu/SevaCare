-- =============================================================
-- The starter catalog: a common Indian OTC/Rx shelf, one sample supplier
-- and opening stock, so a new pharmacy can search and sell on day one
-- instead of staring at an empty search box.
--
-- Run by PharmacyCatalogSeeder, NOT by Flyway. It used to be tenant
-- migration V6, which gated every row on public.tenant_registry saying the
-- tenant had pharmacy on -- but provisionTenant migrates the schema before
-- it commits the registry row, so the gate read NULL and seeded nothing for
-- every tenant onboarded through the app. Seeding is a fact about a tenant's
-- modules, and modules are only knowable after the tenant is committed;
-- that makes it a service's job, not a migration's.
--
-- ${tenantSchema} and ${tenantPublicId} are substituted by the seeder, which
-- runs the whole file in one transaction and only on a store whose catalog
-- and ledger are both still empty.
-- =============================================================
...
SELECT '${tenantPublicId}', 'SKU-SEED0001', 'BATCH-SEED0001', 'COUNTER', 300, 'OPENING',
       'SEED', 'STARTER_CATALOG', 'system_seed', 'Starter catalog opening stock'
;
